...
* Why ? 공개 Key 생성시 전산(CPU)작업을 너무 많이 사용한다.
...
### 2. 공개키와 대칭키가 인터넷 환경에서 사용되는 기본 원리(SSL)
* PC와 Server사이에서 이루어지는 공개키를 사용한 대칭키 교환 과정은 아래와 같다.

* 통신 전 미리 키쌍 생성 -> HDD에 저장
* 효율, CPU연산, 보안성 : 대칭키 > 공개키
  
| 순서  | PC  | Server |
|---|---|---|
| 1. 키 생성  | PC Sym Key (대칭키 생성) | S Pub, Pri key (공개키 쌍 생성) |
| 2. 공개키 교환(서버->PC)| S pub key| |
| 3-1. 대칭키 암호화| PC Sym Key를 S pub key로 암호화||
| 3-2. 대칭키 전달||암호화된 PC Sym Key를 S pri key로 복호화|
| 4. 대칭키 교환 완료| | PC Sym Key |
| 5-1. PC에서 문서 전달| PC Sym key로 문서 암호화||
| 5-2. 서버에서 문서 받음|| PC Sym key로 문서 복호화|

* 공개키 + 대칭키를 합쳐서 사용함으로 대칭키의 키 배송 문제를 해결할 수 있다.
* 효율성, 보안성이 높아진다(PC마다 다른 키 사용) -> 성능이 좋아진다.
* 공개키는 키 생성시 연산 작업이 많이 사용되는데 미리 키를 생성하니 키 생성에 대한 이슈로 인한 지연도가 낮아진다.
* Symmetric key = session key 라고도 한다.
* 대칭키를 공개키로 안전하게 키 배송이 되면 이제 세션이 만료될 때 까지 대칭키로 문서를 암호화, 복호화한다.
...
| 1-1. CA key |  | CA Pub(인증서), Pri key |
| 1-2. RA | | RA Pub(인증서), pri key |
| 1-3. 개인 공개키 생성(RA에 요청) | | 개인 Pub, Pri key |
| 2. 공개 키 교환| 개인 Pub key |  |
| 3-1. 대칭키 암호화| 은행 Sym Key를 개인 pub key로 암호화||
| 3-2. 대칭키 전달||암호화된 은행 Sym Key를 개인 pri key로 복호화|
| 4. 대칭키 교환 완료| | 은행 Sym Key |
| 5-1. PC에서 문서 전달| 은행 Sym key로 문서 암호화||
| 5-2. 서버에서 문서 받음|| 은행 Sym key로 문서 복호화|

* PC <-> 서버의 위치를 바꾸면 은행 <-> 개인이 된다.
...
## 해시 알고리즘
### 해시함수
* 메시지가 해시 함수를 거쳤을 때, 어떠한 값으로 나오는 것
* 무결성을 제공하는 기술
* "메시지 인증"에 사용
* 암호 알고리즘에는 키가 사용되지만, 해시 알고리즘 함수에는 키를 사용하지 않습니다.

### 일방향 해시함수 
* 해시값을 통해 데이터를 알아낼 수 없는 것
* 특징
  * 임이의 길이의 메시지로부터 고정 길이의 해시값을 계산한다.
  * 해시값을 고속으로 계산할 수 있다.
  * 일방향성을 갖는다
  * 메시지가 다르면 해시값도 다르다
  * 충돌 방지(collision resistance)
    * 충돌을 발견하는 것이 어려운 성질을 가져야한다.
   
### 해시함수가 가져야 할 성질
* p.78
* 프리이미지 저항성(Preimage resistance), 역상저항성
    * 
* 제2프리이미지 저항성(second preimage resistance), 약한 충돌 저항성
    * 
...
* 출력 길이에 따라 SHA-256, 512 등으로 나뉜다. (Sha-256 : 문자열을 256bit길이의 key로 변환)


| 구분  | SAH-1  | SHA-2  | | | |
|---|---|---|---|---|---|
|||SHA-224|SHA-256|SHA-384|SHA-512|
| MD길이(output bits) | 160  | 224  | 256 | 384 | 512 |
...
### 빅앤디안
* 최상위 비트(MSB)부터 부호화되어 저장된다.
* 12 -> 1100(2)

### 리틀앤디안
* 최하위 (LSB)부터 부호화되어 저장된다.
* 12 -> 0011(2)
